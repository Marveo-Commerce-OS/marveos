...
export default function MasterClientsPage() {
  ...
  const clients = useMemo(() => {
    ...
    const list = Array.from(rows.values()).sort((a, b) => b.latestUpdatedAt.localeCompare(a.latestUpdatedAt));
    if (!term) return list;
  ...
  }, [workspaces, subscriptionByEmail, subscriptionById, searchTerm]);
  ...
  async function load() {
    ...
    try {
      const [workspaceRes, billingRes, usersRes, resetCapabilitiesRes] = await Promise.all([
        ...
        fetch('/api/master/operations/reset-clients-workspaces', { cache: 'no-store' }),
      ]);
      ...
      const resetData = (await resetCapabilitiesRes.json().catch(() => null)) as { canReset?: boolean; error?: string } | null;
      if (!resetCapabilitiesRes.ok) throw new Error(resetData?.error || 'Failed to load maintenance capabilities');
      ...
      setCanReset(Boolean(resetData?.canReset));
    } catch (err) {
  ...
  async function resetClientAndWorkspaceRecords() {
    if (resetConfirmText.trim().toUpperCase() !== 'RESET') {
      setError('Reset canceled. You must type RESET to confirm.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/master/operations/reset-clients-workspaces', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok || !data?.ok) {
        throw new Error(data?.error || 'Failed to reset client/workspace records.');
      }

      const deleted = data.deletedCounts;
      setMessage(
        `Reset complete. Deleted ${deleted?.workspaces || 0} workspace(s), ${deleted?.clientOrganizations || 0} organization(s), and ${deleted?.subscriptions || 0} subscription(s).`,
      );
      setShowResetConfirm(false);
      setResetConfirmText('');
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to reset client/workspace records.');
    } finally {
      setResetBusy(false);
    }
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canReset ? (
            <button
              ...
              title="Purge all client/workspace/commercial records. Super admin only."
            >
              {showResetConfirm ? 'Cancel reset' : 'Reset clients & workspaces'}
            </button>
          ) : null}
        </div>
      </div>
      ...
      {canReset && showResetConfirm ? (
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4">
          <p className="text-sm font-semibold text-rose-900">Confirm destructive reset</p>
          <p className="mt-1 text-sm text-rose-800">
            Type RESET to purge all client/workspace/commercial records. This action cannot be undone.
          </p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => void resetClientAndWorkspaceRecords()}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Filter clients</label>
        <input
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          placeholder="Search by organization, owner email, country, plan"
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {clients.map((client) => {
              ...
              return (
                <div key={client.key} className="rounded-xl border border-slate-200 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span className={`rounded-full px-2 py-1 text-xs font-semibold ${hasOpenSupport ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                      {hasOpenSupport ? `${client.supportOpenCount} open support` : 'Support clear'}
                    </span>
                  </div>

                  <div className="mt-3 grid gap-3 text-sm text-slate-700 md:grid-cols-2 xl:grid-cols-4">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Geo / Currency</p>
                      <p>{client.country} / {client.currency}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Plan / Subscription</p>
                      <p>{client.planId || '—'}</p>
                      <p className="text-xs text-slate-500">{toLabel(client.subscriptionStatus)}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Trial End</p>
                      <p>{client.trialEndDate ? new Date(client.trialEndDate).toLocaleDateString() : '—'}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Workspaces</p>
                      <p>{client.workspaceCount}</p>
                    </div>
                  </div>
