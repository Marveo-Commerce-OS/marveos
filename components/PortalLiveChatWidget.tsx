...
export default function PortalLiveChatWidget() {
    ...
    void (async () => {
      try {
        ...
        if (!res.ok || !payload || !('ok' in payload)) {
          throw new Error((payload as { error?: string } | null)?.error || 'Unable to load support context.');
        }
        setContext(payload);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Unable to load support context.');
      }
    })();
  }, []);
  ...
  async function loadThread(nextTicketId: string) {
    ...
    try {
      ...
      const payload = (await res.json().catch(() => null)) as { ticket?: { ticketNumber?: string }; messages?: TicketMessage[]; error?: string } | null;
      if (!res.ok || !payload?.ticket) {
        throw new Error(payload?.error || 'Unable to load chat thread.');
      }
      setTicketNumber(String(payload.ticket.ticketNumber || ''));
  ...
  async function startChat(existingTicketAction?: 'update' | 'create_new') {
    if (!context?.workspace.id || !subject.trim() || !startMessage.trim()) {
      setError('Subject and first message are required.');
      return;
    }
    if (requiresPin && !supportPinInput.trim()) {
    ...
    try {
      ...
      setStartMessage('');
      setNotice(payload.reusedExistingTicket ? 'Updated your existing open ticket.' : 'Live chat started.');
  ...
  async function sendReply() {
    ...
    try {
      const res = await fetch(`/api/os/tickets/${encodeURIComponent(ticketId)}/reply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ messageHtml: `<p>${replyMessage.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br/>')}</p>` }),
      });
  ...
  async function endChat() {
    ...
    setError('');
    try {
      const res = await fetch(`/api/os/tickets/${encodeURIComponent(ticketId)}/end`, { method: 'POST' });
      const payload = (await res.json().catch(() => null)) as { ok?: boolean; error?: string } | null;
      if (!res.ok || !payload?.ok) {
      ...
      {!open ? (
        ...
        <div className="w-[min(94vw,420px)] overflow-hidden rounded-3xl border border-sky-100 bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-700">Portal Support</p>
          ...
          <div className="max-h-[76vh] overflow-y-auto p-4">
            {!ticketId ? (
              <div className="space-y-3">
                <div className="rounded-xl border border-sky-100 bg-sky-50 px-3 py-2 text-xs text-sky-700">Workspace: {context?.workspace.name || 'Loading...'}</div>
                ...
                {requiresPin ? (
                  <>
                    <input value={supportPinInput} onChange={(e) => setSupportPinInput(e.target.value)} className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Support PIN (required)" />
                    {context?.maskedSupportPin || revealedSupportPin ? (
                      <div className="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-xs text-sky-700">
                        PIN: {revealedSupportPin || context?.maskedSupportPin}
                        {!revealedSupportPin && context?.canRevealSupportPin ? (
                          <button type="button" onClick={() => void revealSupportPin()} className="ml-2 rounded-full border border-sky-200 bg-white px-2 py-0.5 font-semibold">Reveal</button>
                        ) : null}
                      </div>
                    ) : null}
                  </>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-end gap-2 pt-2">
                  ...
                  <button type="button" onClick={() => void sendReply()} disabled={busy || !replyMessage.trim()} className="rounded-2xl bg-sky-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-60">Send</button>
                </div>
